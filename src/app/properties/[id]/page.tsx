...
export default function PropertyDetailsPage({
  ...
  const [propertyId, setPropertyId] = useState<string | null>(null);
  const router = useRouter();
  const shareRef = useRef<HTMLDivElement>(null); // Ref for click-outside detection
  // const shareRef = useRef<HTMLDivElement>(null); // Added ref for share dropdown
  ...
  // Added useEffect to handle clicking outside the share dropdown
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      if (shareRef.current && !shareRef.current.contains(event.target as Node)) {
        setShowShareOptions(false);
          ...
          <div className="p-6 md:p-8">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
              <div className="mb-4 md:mb-0 flex-grow">
                ...
                {/* Location only - share button moved to better UX position */}
              ...
              <div className="flex items-center gap-4">
                ...
                {/* REPOSITIONED: Share button next to price for better UX - more discoverable */}
                <div className="relative" ref={shareRef}>
                  <button
                    onClick={() => setShowShareOptions(!showShareOptions)}
                    className="flex items-center px-3 py-2 bg-transparent  hover:bg-blue-700 text-blue rounded-lg transition-colors"
                  >
                    <Share size={16} className="mr-2" />
                    Share
                  </button>
